...
mux output/tmp/video_caps.mp4  output/mix_standard.wav output/northwind_repairs_document_intake_final.mp4
mux output/tmp/video_clean.mp4 output/mix_standard.wav output/northwind_repairs_document_intake_no_captions.mp4
# Alternate SFX mixes as drop-in audio tracks (same picture; keeps the repo small)
for v in lighter stronger; do
  ffmpeg -loglevel error -y -i output/mix_$v.wav -af "loudnorm=I=-16:TP=-1.5:LRA=11" -ar 48000 -c:a aac -b:a 192k output/northwind_repairs_document_intake_audio_sfx_$v.m4a
done
if [ "${VARIANT_MP4:-0}" = "1" ]; then
  mux output/tmp/video_caps.mp4 output/mix_lighter.wav  output/northwind_repairs_document_intake_sfx_lighter.mp4
  mux output/tmp/video_caps.mp4 output/mix_stronger.wav output/northwind_repairs_document_intake_sfx_stronger.mp4
fi
echo BUILD_DONE
