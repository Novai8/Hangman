...
export const BIZ = 'Northwind Studio';
export const SAM = { name: 'Sam Rivera', ini: 'SR', email: 'sam.rivera@clientmail.example', color: '#4C6FFF' };
export const SERVICES = ['Consultation', 'Repair visit', 'Installation'];
// Locked request text (split into lines for the message card)
export const REQUEST = ['Hi, I’d like a consultation.', 'Next week works, preferably after 3pm.', 'This is urgent.'];
export const TZ = 'Local time';
export const CAL = { team: 'Team calendar', avail: 'Availability' };
export const SLOTS = ['Tue 4:00', 'Wed 3:30', 'Thu 5:00'];
export const CHOSEN = 1; // Sam picks "Wed 3:30"
export const DAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
export const ROWS = ['2:00', '2:30', '3:00', '3:30', '4:00', '4:30', '5:00', '5:30']; // pm, Local time
// ---------------------------------------------------------------- tag styles
export const TAG = {
...
export function appWindow(x, y, w, h, title, opt = {}) {
  ...
  if (opt.owner !== false && opt.owner !== null) {
    chip(opt.owner || 'Northwind Studio', x + w - 20, y + 30, { align: 'right', size: 18, h: 36, pad: 14, bg: 'rgba(17,24,39,0.05)', fg: '#374151', icon: ICON.user });
  }
}
...
/** Horizontal stepper for the six workflow stages. active: index; done < active get a check. */
export const STEPS = ['Extract details', 'Check availability', 'Propose slots', 'Confirm booking', 'Reminders', 'Log'];
export function stepper6(t, marks, a = 1, y = 140) {
  // marks: time each step becomes active
  if (a <= 0) return;
  const size = 19, h = 40, pad = 16, gap = 36;
  ...
  withAlpha(a, () => {
    STEPS.forEach((l, i) => {
      ...
      const pp = E.outExpo(prog(t, marks[i], 0.4));
      const bg = on ? hexA('#4C6FFF', 0.12 + 0.02 * pp) : done ? hexA('#19B89D', 0.12) : 'rgba(17,24,39,0.05)';
